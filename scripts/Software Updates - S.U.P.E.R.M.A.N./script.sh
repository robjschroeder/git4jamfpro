...
echo "
  ...
  [--display-redraw=seconds] [--defer-display-timeout=seconds]
  [--soft-display-timeout=seconds] [--display-icon=/local/path or URL]
  ...
  [--user-auth-mdm-failover=ALWAYS,SOFT,HARD,INSTALLNOW,BOOTSTRAP]
  
  Update, Upgrade, and Restart Options:
  [--allow-upgrade] [--allow-upgrade-off] [--target-upgrade=version]
  [--enforce-all-updates] [--enforce-all-updates-off] [--only-download]
  [--only-download-off] [--install-now] [--install-now-off]
  ...
  * Managed preferences override local options via domain: com.macjutsu.super
  ...
  <key>DeferDisplayTimeout</key> <string>seconds</string>
  <key>SoftDisplayTimeout</key> <string>seconds</string>
  ...
  <string>ALWAYS,SOFT,HARD,INSTALLNOW,BOOTSTRAP</string>
  <key>AllowUpgrade</key> <true/> | <false/>
  <key>TargetUpgrade</key> <string>version</string>
  <key>EnforceAllUpdates</key> <true/> | <false/>
...
# IMPORTANT DETAIL: super will NOT move the default erase-install folder content to another custom location.
...
while [[ -n $1 ]]; do
	case "$1" in
		...
		;;
		--defer-display-timeout* )
			deferDisplayTimeoutOPTION=$(echo "$1" | sed -e 's|^[^=]*=||g')
		;;
		--soft-display-timeout* )
			softDisplayTimeoutOPTION=$(echo "$1" | sed -e 's|^[^=]*=||g')
		;;
		--display-icon* )
		...
		;;
		-A|--enforce-all-updates|--enforce-all-updates-on )
			enforceAllUpdatesOPTION="TRUE"
		;;
		-a|--enforce-all-updates-off|--no-enforce-all-updates )
			enforceAllUpdatesOPTION="FALSE"
		;;
...
if [[ "$resetLocalPROPERTIES" == "TRUE" ]]; then
	...
	defaults delete "$superPLIST" DeferDisplayTimeout 2> /dev/null
	defaults delete "$superPLIST" SoftDisplayTimeout 2> /dev/null
	...
	defaults delete "$superPLIST" EnforceAllUpdates 2> /dev/null
...
if [[ -f "$superMANAGEDPLIST.plist" ]]; then
	...
	deferDisplayTimeoutMANAGED=$(defaults read "$superMANAGEDPLIST" DeferDisplayTimeout 2> /dev/null)
	softDisplayTimeoutMANAGED=$(defaults read "$superMANAGEDPLIST" SoftDisplayTimeout 2> /dev/null)
	...
	enforceAllUpdatesMANAGED=$(defaults read "$superMANAGEDPLIST" EnforceAllUpdates 2> /dev/null)
...
if [[ -f "$superPLIST.plist" ]]; then
	...
	deferDisplayTimeoutPROPERTY=$(defaults read "$superPLIST" DeferDisplayTimeout 2> /dev/null)
	softDisplayTimeoutPROPERTY=$(defaults read "$superPLIST" SoftDisplayTimeout 2> /dev/null)
	...
	enforceAllUpdatesPROPERTY=$(defaults read "$superPLIST" EnforceAllUpdates 2> /dev/null)
...
if [[ -n $deferDisplayTimeoutMANAGED ]]; then
	deferDisplayTimeoutOPTION="$deferDisplayTimeoutMANAGED"
elif [[ -z $deferDisplayTimeoutOPTION ]] && [[ -n $deferDisplayTimeoutPROPERTY ]]; then
	deferDisplayTimeoutOPTION="$deferDisplayTimeoutPROPERTY"
fi
if [[ -n $softDisplayTimeoutMANAGED ]]; then
	softDisplayTimeoutOPTION="$softDisplayTimeoutMANAGED"
elif [[ -z $softDisplayTimeoutOPTION ]] && [[ -n $softDisplayTimeoutPROPERTY ]]; then
	softDisplayTimeoutOPTION="$softDisplayTimeoutPROPERTY"
fi
...
if [[ -n $enforceAllUpdatesMANAGED ]]; then
	enforceAllUpdatesOPTION="$enforceAllUpdatesMANAGED"
elif [[ -z $enforceAllUpdatesOPTION ]] && [[ -n $enforceAllUpdatesPROPERTY ]]; then
	enforceAllUpdatesOPTION="$enforceAllUpdatesPROPERTY"
fi
...
if [[ "$errorDeferOPTION" == "X" ]]; then
	...
	defaults write "$superPLIST" DefaultDefer -string "$errorDeferSECONDS"
elif [[ -n $errorDeferOPTION ]] && ! [[ $errorDeferOPTION =~ $regexNUMBER ]]; then
...
# Validate $deferDisplayTimeoutOPTION and if valid set $deferDisplayTimeoutSECONDS.
if [[ "$deferDisplayTimeoutOPTION" == "X" ]]; then
	sendToLog "Startup: Deleting local preference for restart/defer display timeout."
	defaults delete "$superPLIST" DeferDisplayTimeout 2> /dev/null
elif [[ -n $deferDisplayTimeoutOPTION ]] && [[ $deferDisplayTimeoutOPTION =~ $regexNUMBER ]]; then
	if [[ $deferDisplayTimeoutOPTION -lt 60 ]]; then
		sendToLog "Warning: Specified restart/defer display timeout of $deferDisplayTimeoutOPTION seconds is too low, rounding up to 60 seconds."
		deferDisplayTimeoutSECONDS=60
	elif [[ $deferDisplayTimeoutOPTION -gt 86400 ]]; then
		sendToLog "Warning: Specified restart/defer display timeout of $deferDisplayTimeoutOPTION seconds is too high, rounding down to 86400 seconds (1 day)."
		deferDisplayTimeoutSECONDS=86400
	else
		deferDisplayTimeoutSECONDS=$deferDisplayTimeoutOPTION
	fi
	defaults write "$superPLIST" DeferDisplayTimeout -string "$deferDisplayTimeoutSECONDS"
elif [[ -n $deferDisplayTimeoutOPTION ]] && ! [[ $deferDisplayTimeoutOPTION =~ $regexNUMBER ]]; then
	sendToLog "Parameter Error: The restart/defer display timeout must only be a number."; parameterERROR="TRUE"
fi
if [[ "$parameterERROR" != "TRUE" ]] && [[ -n $deferDisplayTimeoutSECONDS ]] && [[ -n $displayRedrawSECONDS ]]; then
	displayMinimumTIMEOUT=$((displayRedrawSECONDS * 3))
	if [[ $deferDisplayTimeoutSECONDS -lt $displayMinimumTIMEOUT ]];then
		sendToLog "Warning: Specified restart/defer display timeout of $deferDisplayTimeoutSECONDS seconds is too low given a display redraw of $displayRedrawSECONDS seconds, changing restart/defer display timeout to $displayMinimumTIMEOUT seconds."
		deferDisplayTimeoutSECONDS=$displayMinimumTIMEOUT
		defaults write "$superPLIST" DeferDisplayTimeout -string "$deferDisplayTimeoutSECONDS"
	fi
fi

# Validate $softDisplayTimeoutOPTION and if valid set $softDisplayTimeoutSECONDS.
if [[ "$softDisplayTimeoutOPTION" == "X" ]]; then
	sendToLog "Startup: Deleting local preference for soft deadline display timeout."
	defaults delete "$superPLIST" DeferDisplayTimeout 2> /dev/null
elif [[ -n $softDisplayTimeoutOPTION ]] && [[ $softDisplayTimeoutOPTION =~ $regexNUMBER ]]; then
	if [[ $softDisplayTimeoutOPTION -lt 60 ]]; then
		sendToLog "Warning: Specified soft deadline display timeout of $softDisplayTimeoutOPTION seconds is too low, rounding up to 60 seconds."
		softDisplayTimeoutSECONDS=60
	elif [[ $softDisplayTimeoutOPTION -gt 86400 ]]; then
		sendToLog "Warning: Specified soft deadline display timeout of $softDisplayTimeoutOPTION seconds is too high, rounding down to 86400 seconds (1 day)."
		softDisplayTimeoutSECONDS=86400
	else
		softDisplayTimeoutSECONDS=$softDisplayTimeoutOPTION
	fi
	defaults write "$superPLIST" DeferDisplayTimeout -string "$softDisplayTimeoutSECONDS"
elif [[ -n $softDisplayTimeoutOPTION ]] && ! [[ $softDisplayTimeoutOPTION =~ $regexNUMBER ]]; then
	sendToLog "Parameter Error: The soft deadline display timeout must only be a number."; parameterERROR="TRUE"
fi
if [[ "$parameterERROR" != "TRUE" ]] && [[ -n $softDisplayTimeoutSECONDS ]] && [[ -n $displayRedrawSECONDS ]]; then
	displayMinimumTIMEOUT=$((displayRedrawSECONDS * 3))
	if [[ $softDisplayTimeoutSECONDS -lt $displayMinimumTIMEOUT ]];then
		sendToLog "Warning: Specified soft deadline display timeout of $softDisplayTimeoutSECONDS seconds is too low given a display redraw of $displayRedrawSECONDS seconds, changing soft deadline display timeout to $displayMinimumTIMEOUT seconds."
		softDisplayTimeoutSECONDS=$displayMinimumTIMEOUT
		defaults write "$superPLIST" DeferDisplayTimeout -string "$softDisplayTimeoutSECONDS"
	fi
fi
...
if [[ -n $displayAccessoryTypeOPTION ]] && [[ -z $displayAccessoryDefaultOPTION ]] && [[ -z $displayAccessoryUpdateOPTION ]] && [[ -z $displayAccessoryUpgradeOPTION ]]; then
	sendToLog "Parameter Error: To use a display accessory type you must also specifiy one of the display accessory content options."; parameterERROR="TRUE"
fi
if [[ -z $displayAccessoryTypeOPTION ]] && { [[ -n $displayAccessoryDefaultOPTION ]] || [[ -n $displayAccessoryUpdateOPTION ]] || [[ -n $displayAccessoryUpgradeOPTION ]]; }; then
	sendToLog "Parameter Error: To use any of the display accessory content options you must also specifiy the display accessory type."; parameterERROR="TRUE"
fi
...
if [[ "$parameterERROR" != "TRUE" ]] && { [[ -n $displayAccessoryDefaultOPTION ]] || [[ -n $displayAccessoryUpdateOPTION ]] || [[ -n $displayAccessoryUpgradeOPTION ]]; }; then
...
if [[ "$helpButtonOPTION" == "X" ]]; then
	sendToLog "Startup: Deleting local preference for helb button option."
	defaults delete "$superPLIST" HelpButton 2> /dev/null
...
if [[ "$userAuthMDMFailoverOPTION" == "X" ]]; then
	...
	for option in "${userAuthMDMFailoverARRAY[@]}"; do
		if [[ $option =~ ^ALWAYS$ ]]; then
			...
			sendToLog "Parameter Error: Unrecognized user authentication MDM failover type: $option. You can specify any of the following types seperated by commas (no spaces); ALWAYS,SOFT,HARD,INSTALLNOW,BOOTSTRAP."; parameterERROR="TRUE"
		fi
	done
	IFS="$oldIFS"
...
# Manage $enforceAllUpdatesOPTION and save to $superPLIST.
if [[ $enforceAllUpdatesOPTION -eq 1 ]] || [[ "$enforceAllUpdatesOPTION" == "TRUE" ]]; then
	enforceAllUpdatesOPTION="TRUE"
	defaults write "$superPLIST" EnforceAllUpdates -bool true
else
	enforceAllUpdatesOPTION="FALSE"
	defaults delete "$superPLIST" EnforceAllUpdates 2> /dev/null
fi
...
if [[ "$skipUpdatesOPTION" == "TRUE" ]] && [[ "$onlyDownloadOPTION" == "TRUE" ]]; then
	sendToLog "Parameter Error: The --force-restart option can not be used with the --only-download option."; parameterERROR="TRUE"
fi

# Manage $forceRestartOPTION and save to $superPLIST.
if [[ $forceRestartOPTION -eq 1 ]] || [[ "$forceRestartOPTION" == "TRUE" ]]; then
	forceRestartOPTION="TRUE"
	defaults write "$superPLIST" ForceRestart -bool true
else
	forceRestartOPTION="FALSE"
	defaults delete "$superPLIST" ForceRestart 2> /dev/null
fi
if [[ "$forceRestartOPTION" == "TRUE" ]] && [[ "$onlyDownloadOPTION" == "TRUE" ]]; then
	sendToLog "Parameter Error: The --force-restart option can not be used with the --only-download option."; parameterERROR="TRUE"
fi
...
if [[ "$parameterERROR" != "TRUE" ]] && [[ -n $displayRedrawSECONDS ]]; then
	displayMinimumTIMEOUT=$((displayRedrawSECONDS * 3))
	if [[ $testModeTimeoutSECONDS -lt $displayMinimumTIMEOUT ]];then
		sendToLog "Warning: Specified test mode timeout of $testModeTimeoutSECONDS seconds is too low given a display redraw of $displayRedrawSECONDS seconds, changing test mode timeout to $displayMinimumTIMEOUT seconds."
		testModeTimeoutSECONDS=$displayMinimumTIMEOUT
		defaults write "$superPLIST" TestModeTimeout -string "$testModeTimeoutSECONDS"
	fi
fi
}
...
# Beta workflow modes: FALSE, PublicSeed, CustomerSeed, or DeveloperSeed
betaWORKFLOW="FALSE"
...
if [[ "$macOSARCH" == "arm64" ]]; then  # Mac computers with Apple Silicon.
	...
	if [[ "$localCREDENTIAL" == "TRUE" ]]; then
		...
		if [[ $macOSVERSION -ge 1103 ]]; then
			...
			sendToLog "Startup: User authentication will be required to perform a macOS update/upgrade."
			updateWORKFLOW="USER"
...
if [[ $(echo "$seedutilSTATUS" | grep -c 'Is Enrolled: YES') -gt 0 ]]; then
	betaWORKFLOW=$(echo "$seedutilSTATUS" | awk '/Currently enrolled in:/ {print $4;}')
	sendToLog "Startup: This system is currently enrolled in beta access program: $betaWORKFLOW"
fi
...
[[ -n $deferDisplayTimeoutSECONDS ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: deferDisplayTimeoutSECONDS is: $deferDisplayTimeoutSECONDS"
[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: softDisplayTimeoutSECONDS is: $softDisplayTimeoutSECONDS"
...
[[ -n $enforceAllUpdatesOPTION ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: enforceAllUpdatesOPTION is: $enforceAllUpdatesOPTION"
...
[[ -n $forceRestartOPTION ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: forceRestartOPTION is: $forceRestartOPTION"
...
storageREADY="FALSE"
availableStorageGB=$(osascript -l 'JavaScript' -e "ObjC.import('Foundation'); var freeSpaceBytesRef=Ref(); $.NSURL.fileURLWithPath('/').getResourceValueForKeyError(freeSpaceBytesRef, 'NSURLVolumeAvailableCapacityForImportantUsageKey', null); Math.round(ObjC.unwrap(freeSpaceBytesRef[0]) / 1000000000)")
...
if [[ -z $availableStorageGB ]] || [[ ! $availableStorageGB =~ $regexNUMBER ]]; then
	sendToLog "Warning: Using less-accurate method to determine available free storage."
	availableStorageGB=$(df -Pk . | column -t | sed 1d | awk '{print $4}' | xargs -I{} expr {} / 1000000)
	[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: availableStorageGB: $availableStorageGB"
fi
...
# Validate if current system power is adequate for performing a macOS update/upgrade and set $powerREADY accordingly. Desktops will obviously always return that they are ready.
checkAvailablePower() {
powerREADY="FALSE"
...
# Validate that the computer's bootstrap token is properly escrowed.
checkBootstrapToken() {
commandRESULT=$(profiles status -type bootstraptoken 2>&1)
[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: commandRESULT:\n$commandRESULT"
if [[ $(echo "$commandRESULT" | grep -c 'YES') -eq 2 ]]; then
	sendToLog "Status: Bootstrap token escrow validated."
	bootstrapTOKEN="TRUE"
else
	sendToLog "Warning: Bootstrap token is not escrowed."
		...
		if [[ "$previousMacOSSoftwareUpgradeLABEL" != "0" ]] && [[ "$previousMacOSSoftwareUpgradeTITLE" != "0" ]]; then # macOS upgrade available per cache.
			...
			if [[ "$upgradeWORKFLOW" != "FALSE" ]]; then # macOS upgrade option is enabled.
				if [[ -n $softwareUpdateLIST ]];then # Cached macOS installer list exists.
					if [[ $(echo "$softwareUpdateLIST" | grep -c 'Software Update found') -gt 0 ]] && [[ $(echo "$softwareUpdateLIST" | grep -c 'macOS') -gt 0 ]];then # Cached macOS softareupdate list was completed.
...
# This function evaluates software updates collected from the listSoftwareUpdates() function and sets super software update caches, $softwareUpdateMACOS, and $softwareUpdateRECOMMENDED accordingly.
...
if [[ "$softwareUpdatesAVAILABLE" == "TRUE" ]]; then
	...
	if [[ $macOSMAJOR -ge 12 ]]; then # For macOS 12 softwareupdate can list multiple macOS updates/upgrades.
		...
		macOSSoftwareUpdateGBS=($(echo "$softwareUpdateLIST" | grep 'Title: macOS' | sort -k6 -r -V | awk -F ': ' '{print $4}' | sed -e 's/,//g' | grep -o -E '[0-9]+' | awk '{print $1/1000000}' | awk -F '.' '{print $1+1}'))
		...
		macOSSoftwareUpdateGB=$(echo "$softwareUpdateLIST" | grep 'Title: macOS' | awk -F ': ' '{print $4}' | grep -o -E '[0-9]+' | awk '{print $1/1000000}' | awk -F '.' '{print $1+1}')
		...
		macOSSoftwareUpdateGB=$(echo "$softwareUpdateLIST" | grep 'macOS' | awk -F '(' '/\t/ {print $2}' | grep -o -E '[0-9]+' | awk '{print $1/1000000}' | awk -F '.' '{print $1+1}')
		macOSSoftwareUpdateVERSION=$(echo "$macOSSoftwareUpdateLABEL" | grep -o '10.[0-9][0-9].[0-9]')
	fi
...
if [[ "$fullCheckREQUIRED" == "TRUE" ]]; then
	...
	if [[ "$workflowStartFAIL" == "TRUE" ]] && [[ "$workflowStartTIMEOUT" == "TRUE" ]]; then
		if [[ "$installNowOPTION" == "TRUE" ]]; then
			sendToLog "Error: Checking for macOS instsallers did not complete after multiple attempts, install now workflow can not continue."
			sendToStatus "Inactive Error: Checking for macOS instsallers did not complete after multiple attempts, install now workflow can not continue."
			notifyInstallNowFailure
			errorExit
		else
			deferSECONDS="$errorDeferSECONDS"
			sendToLog "Error: Checking for macOS instsallers did not complete after multiple attempts, trying again in $deferSECONDS seconds."
			sendToStatus "Pending: Checking for macOS instsallers did not complete after multiple attempts, trying again in $deferSECONDS seconds."
			makeLaunchDaemonCalendar
		fi
	fi
fi
...
if { [[ $macOSVERSION -lt 1203 ]] && [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]]; } || { [[ $macOSVERSION -ge 1203 ]] && [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]] && [[ "$upgradeWORKFLOW" == "JAMF" ]]; }; then
	...
	if [[ -n $macOSSoftwareUpdateDownloadLABEL ]] && [[ -n $previousLastREBOOT ]]; then
		...
		if [[ "$lastREBOOT" != "$previousLastREBOOT" ]]; then
			sendToLog "Warning: The system has been restarted without applying the prevously downloaded macOS update, download workflow needs to run again."
			previousDownloadFAILURE="TRUE"
		fi
...
# The softwareupdate download process is backgrounded and will be watched via a while loop later on. Also note the difference between macOS versions.
...
if [[ "$workflowStartTIMEOUT" == "FALSE" ]] && [[ "$workflowStartFAIL" == "FALSE" ]] && [[ "$workflowTIMEOUT" == "FALSE" ]]; then
	...
	if [[ "$softwareUpdateTitleTARGET" == "$softwareUpdateDownloadTITLE" ]]; then
		[[ $macOSMAJOR -ge 11 ]] && preparedMacOSVERSION=$(defaults read /System/Volumes/Update/Update update-asset-attributes 2> /dev/null | grep -w 'OSVersion' | sed -e 's/    OSVersion = "//' -e 's/";//')
		[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: preparedMacOSVERSION is: $preparedMacOSVERSION"
		[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: macOSUpgradeVersionTARGET is: $macOSUpgradeVersionTARGET"
		[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: macOSSoftwareUpdateVERSION is: $macOSSoftwareUpdateVERSION"
		if { [[ $macOSVERSION -ge 1203 ]] && [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]]; } && [[ "$preparedMacOSVERSION" != "$macOSUpgradeVersionTARGET" ]]; then
...
sendToLog "erase-install.sh: Starting $macOSInstallerNameTARGET $macOSInstallerVersionTARGET-$macOSInstallerBuildTARGET download instaler workflow, check $installerLOG for more detail."
sendToStatus "Running: erase-install.sh: Starting $macOSInstallerNameTARGET $macOSInstallerVersionTARGET-$macOSInstallerBuildTARGET download instaler workflow."
	...
	if [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]]; then # A macOS upgrade is available and option to allow upgrade is enabled.
		if [[ $macOSVERSION -ge 1203 ]] && [[ "$upgradeWORKFLOW" != "JAMF" ]]; then # macOS 12.3 or newer can upgrade via softwareupdate, unless using MDM workflow.
			if [[ "$macOSSoftwareUpdateDownloadREQUIRED" == "TRUE" ]]; then
				...
				sendToLog "Status: Previoulsy downloaded macOS upgrade is prepared: $macOSSoftwareUpdateDownloadLABEL"
			fi
		...
		if [[ "$macOSSoftwareUpdateDownloadREQUIRED" == "TRUE" ]]; then
			...
			sendToLog "Status: Previoulsy downloaded macOS update is prepared: $macOSSoftwareUpdateDownloadLABEL"
		fi
	fi
else # Test mode.
	if [[ "$installNowOPTION" == "TRUE" ]]; then
...
sendToASULog "**** S.U.P.E.R.M.A.N. $superVERSION INSTALL RECOMMENDED UPDATES SOFTWAREUPDATE START ****"
oldIFS="$IFS"; IFS=$' '
[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: recommendedSoftwareUpdateLABELS[] is:\n${recommendedSoftwareUpdateLABELS[*]}"
...
if [[ "$workflowStartTIMEOUT" == "FALSE" ]] && [[ "$workflowStartFAIL" == "FALSE" ]] && [[ "$workflowTIMEOUT" == "FALSE" ]]; then
	...
	if [[ ! $(echo -e "${recommendedSoftwareUpdateTITLES[*]}\n${softwareUpdateInstalledTITLES[*]}" | sort | uniq -u) ]]; then
		sendToASULog "**** S.U.P.E.R.M.A.N. $superVERSION RECOMMENDED UPDATES SOFTWAREUPDATE COMPLETED ****"
		...
		sendToASULog "**** S.U.P.E.R.M.A.N. $superVERSION RECOMMENDED UPDATES SOFTWAREUPDATE INCOMPLETE ****"
	...
	sendToASULog "**** S.U.P.E.R.M.A.N. $superVERSION RECOMMENDED UPDATES SOFTWAREUPDATE FAILED ****"
	if [[ "$workflowStartTIMEOUT" == "TRUE" ]]; then
...
if [[ "$testModeOPTION" != "TRUE" ]]; then # Not in test mode.
	...
	if [[ $(echo "$commandRESULT" | grep -c '200') -gt 0 ]] || [[ $(echo "$commandRESULT" | grep -c '201') -gt 0 ]]; then
		...
		if [[ "$installNowOPTION" == "TRUE" ]]; then
			if [[ "$userAuthMDMFAILOVER" == "TRUE" ]]; then
				sendToLog "Error: Push workflow for macOS update/upgrade via MDM failed, failing over to user authenticated workflow."
				sendToStatus "Inactive Error: Push workflow for macOS update/upgrade via MDM failed, failing over to user authenticated workflow."
				upgradeWORKFLOW="MDMFAIL"
				updateWORKFLOW="MDMFAIL"
				unset policyTRIGGERS
				installRestartWorkflowActiveUser
			...
			if [[ "$currentUserNAME" != "FALSE" ]] && [[ "$userAuthMDMFAILOVER" == "TRUE" ]]; then
				sendToLog "Error: Push workflow for macOS update/upgrade via MDM failed, failing over to user authenticated workflow."
				sendToStatus "Pending: Push workflow for macOS update/upgrade via MDM failed, failing over to user authenticated workflow."
				upgradeWORKFLOW="MDMFAIL"
				updateWORKFLOW="MDMFAIL"
				unset policyTRIGGERS
				installRestartWorkflowActiveUser
...
# This is the install workflow for all (non-macOS) updates when enforced.
...
if [[ "$testModeOPTION" != "TRUE" ]]; then
	...
	if [[ "$softwareUpdateERROR" == "TRUE" ]]; then
		...
		if [[ "$softwareUpdateERROR" == "TRUE" ]]; then
			if [[ "$installNowOPTION" == "TRUE" ]]; then
				sendToLog "Error: Failed to install all recommended (non-macOS) updates after multiple attempts, install now workflow can not continue."
				sendToStatus "Inactive Error: Failed to install all recommended (non-macOS) updates after multiple attempts, install now workflow can not continue."
				notifyInstallNowFailure
				errorExit
			else
				deferSECONDS="$errorDeferSECONDS"
				sendToLog "Warning: Failed to install all recommended (non-macOS) updates after multiple attempts, trying again in $deferSECONDS seconds."
				sendToStatus "Pending: Failed to install all recommended (non-macOS) updates after multiple attempts, trying again in $deferSECONDS seconds."
				makeLaunchDaemonCalendar
			fi
		else
			fullCheckREQUIRED="TRUE"
	...
	else # Some software udpates did not complete
		if [[ "$installNowOPTION" == "TRUE" ]]; then
			sendToLog "Error: Some recommended (non-macOS) updates did not complete, install now workflow can not continue."
			sendToStatus "Inactive Error: Some recommended (non-macOS) updates did not complete, install now workflow can not continue."
			notifyInstallNowFailure
			errorExit
		else
			deferSECONDS="$errorDeferSECONDS"
			sendToLog "Warning: Some recommended (non-macOS) updates did not complete, trying again in $deferSECONDS seconds."
			sendToStatus "Pending: Some recommended (non-macOS) updates did not complete, trying again in $deferSECONDS seconds."
			makeLaunchDaemonCalendar
		fi
	fi
else # Test mode.
...
if [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]]; then # A macOS upgrade is available and option to allow upgrade is enabled.
	if [[ "$upgradeWORKFLOW" == "LOCAL" ]]; then
		...
		checkBootstrapToken
		if [[ "$bootstrapTOKEN" == "TRUE" ]]; then
			[[ -n $policyTRIGGERS ]] && runJamfPolicies
			downloadMacOSInstaller
			installMacOSMDM
		else # Cannot enforce or fix missing bootstrap token.
			deferSECONDS="$errorDeferSECONDS"
...
[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: installNowOPTION is: $installNowOPTION"
if [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]]; then # A macOS upgrade is available and option to allow upgrade is enabled.
	# If MDM workflow is expected, first check for bootstrap token and possibly failover to user authentication workflow.
	if [[ "$upgradeWORKFLOW" == "JAMF" ]]; then
		checkBootstrapToken
		if [[ "$bootstrapTOKEN" != "TRUE" ]]; then
			if [[ "$userAuthMDMFAILOVER" == "TRUE" ]] || [[ "$userAuthMDMFailoverBOOTSTRAP" == "TRUE" ]] || { [[ "$installNowOPTION" == "TRUE" ]] && [[ "$userAuthMDMFailoverINSTALLNOW" == "TRUE" ]]; }; then
				sendToLog "Status: User authentication failover active due to missing bootstrap token escrow."
				upgradeWORKFLOW="MDMFAIL"
			else
				if [[ "$installNowOPTION" == "TRUE" ]]; then
...
else # Only recommended (non-macOS) updates are available.
...
#### Language for notifyRecommended(), a non-interactive notification informing the user that the install now workflow is installing recommended (non-macOS) software updates.
...
dialogUserAuthDeadlineDaysBODY="You have deferred the maximum number of $hardDaysMAX days.\n\nYou must enter the password for user \"$currentUserRealNAME\" to install the $titleWorkflowDISPLAY.\n"
...
dialogUserAuthRetryDeadlineDaysBODY="You have deferred the maximum number of $hardDaysMAX days.\n\nYou must enter the password for user \"$currentUserRealNAME\" to install the $titleWorkflowDISPLAY.\n"
...
# For IBM Notifier dialogs set a new $deferButtonDISPLAY showing the $defaultDeferSECONDS.
setDeferButton() {
if [[ $defaultDeferSECONDS -lt 3600 ]]; then
	deferButtonDISPLAY="$deferForButtonDISPLAY $((defaultDeferSECONDS / 60)) $dialogRestartOrDeferMinutesDISPLAY"
elif [[ $defaultDeferSECONDS -eq 3600 ]]; then
	deferButtonDISPLAY="$deferForButtonDISPLAY 1 $dialogRestartOrDeferHourDISPLAY"
elif [[ $defaultDeferSECONDS -gt 3600 ]] && [[ $defaultDeferSECONDS -lt 7200 ]]; then
	deferButtonDISPLAY="$deferForButtonDISPLAY 1 $dialogRestartOrDeferHourDISPLAY $((defaultDeferSECONDS % 3600 / 60)) $dialogRestartOrDeferMinutesDISPLAY"
elif [[ $defaultDeferSECONDS -ge 7200 ]] && [[ $defaultDeferSECONDS -lt 86400 ]] && [[ $((defaultDeferSECONDS % 3600)) -eq 0 ]]; then
	deferButtonDISPLAY="$deferForButtonDISPLAY $((defaultDeferSECONDS / 3600)) $dialogRestartOrDeferHoursDISPLAY"
elif [[ $defaultDeferSECONDS -gt 7200 ]] && [[ $defaultDeferSECONDS -lt 86400 ]] && [[ $((defaultDeferSECONDS % 3600)) -ne 0 ]]; then
	deferButtonDISPLAY="$deferForButtonDISPLAY $((defaultDeferSECONDS / 3600)) $dialogRestartOrDeferHoursDISPLAY $((defaultDeferSECONDS % 3600 / 60)) $dialogRestartOrDeferMinutesDISPLAY"
elif [[ $defaultDeferSECONDS -eq 86400 ]]; then
		...
		if [[ "$customDisplayACCESSORY" == "TRUE" ]]; then
			if [[ -n $secureAccessoryPAYLOAD ]]; then
				...
				if [[ -n $menuDeferSECONDS ]] && [[ -n $displayTimeoutSECONDS ]]; then
					sendToLog "Warning: Unable to show display timeout countdown due to the custom display accessory option. However, there will still be a display timeout at $displayTimeoutSECONDS seconds."
...
while [[ "$whileTimeoutSECONDS" -ge 0 ]]; do
	...
	if [[ "$storageREADY" == "TRUE" ]]; then
		sendToLog "Status: Current available storage is now at $availableStorageGB GBs, the macOS update/upgade workflow can continue."
		kill -9 "$notifyPID" > /dev/null 2>&1
		if [[ "$ibmNotifierVALID" == "TRUE" ]] && [[ "$preferJamfHelperOPTION" != "TRUE" ]]; then
			killall -9 "IBM Notifier" "IBM Notifier Popup" > /dev/null 2>&1
		else
...
if [[ "$storageREADY" == "FALSE" ]]; then
	if [[ "$installNowOPTION" == "TRUE" ]]; then
		sendToLog "Error: Waiting for user to make more free space avaliable timed out after $workflowTimeoutSECONDS seconds, install now workflow can not continue."
		sendToStatus "Inactive Error: Waiting for user to make more free space avaliable timed out after $workflowTimeoutSECONDS seconds, install now workflow can not continue."
		notifyInstallNowFailure
		errorExit
	else
		deferSECONDS="$errorDeferSECONDS"
		sendToLog "Error: Waiting for user to make more free space avaliable timed out after $workflowTimeoutSECONDS seconds, trying again in $deferSECONDS seconds."
		sendToStatus "Pending: Waiting for user to make more free space avaliable timed out after $workflowTimeoutSECONDS seconds, trying again in $deferSECONDS seconds."
		notifyFailure
		makeLaunchDaemonCalendar
	fi
fi
...
while [[ "$whileTimeoutSECONDS" -ge 0 ]]; do
	...
	[[ "$verboseModeOPTION" == "TRUE" ]] && sendToLog "Verbose Mode: Function ${FUNCNAME[0]}: acPOWER: $acPOWER"
	if [[ "$acPOWER" == "TRUE" ]]; then
		sendToLog "Status: AC power detected, the macOS update/upgade workflow can continue."
		kill -9 "$notifyPID" > /dev/null 2>&1
		if [[ "$ibmNotifierVALID" == "TRUE" ]] && [[ "$preferJamfHelperOPTION" != "TRUE" ]]; then
			killall -9 "IBM Notifier" "IBM Notifier Popup" > /dev/null 2>&1
		else
...
# Display a non-interactive notification informing the user that the install now workflow is installing recommended (non-macOS) software updates.
notifyRecommended() {
setDisplayLanguage
if [[ "$ibmNotifierVALID" == "TRUE" ]] && [[ "$preferJamfHelperOPTION" != "TRUE" ]]; then
	sendToLog "IBM Notifier: Opening install now installing recommended (non-macOS) software updates notification..."
	...
	sendToLog "jamfHelper: Opening install now installing recommended (non-macOS) software updates notification..."
...
[[ -n $deferDisplayTimeoutSECONDS ]] && displayTimeoutSECONDS=$deferDisplayTimeoutSECONDS

if [[ "$ibmNotifierVALID" == "TRUE" ]] && [[ "$preferJamfHelperOPTION" != "TRUE" ]]; then
	[[ -n $deferDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: Ask for restart or defer dialog with a $displayTimeoutSECONDS second timeout."
	[[ -z $deferDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: Ask for restart or defer dialog with no timeout."
	...
	[[ -n $deferDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: Ask for restart or defer dialog with a $displayTimeoutSECONDS second timeout."
	[[ -z $deferDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: Ask for restart or defer dialog with no timeout."
...
[[ -n $softDisplayTimeoutSECONDS ]] && displayTimeoutSECONDS=$softDisplayTimeoutSECONDS
...
if [[ "$ibmNotifierVALID" == "TRUE" ]] && [[ "$preferJamfHelperOPTION" != "TRUE" ]]; then
	...
	if [[ "$deadlineDateSTATUS" == "SOFT" ]]; then
		[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: soft date deadline dialog with a $displayTimeoutSECONDS second timeout."
		[[ -z $softDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: soft date deadline dialog with no timeout."
		ibmNotifierARRAY+=(-subtitle "$dialogSoftDeadlineBodyDateIBM")
	elif [[ "$deadlineDaysSTATUS" == "SOFT" ]]; then
		[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: soft days deadline dialog with a $displayTimeoutSECONDS second timeout."
		[[ -z $softDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: soft days deadline dialog with no timeout."
		ibmNotifierARRAY+=(-subtitle "$dialogSoftDeadlineBodyDaysIBM")
	elif [[ "$deadlineCountSTATUS" == "SOFT" ]]; then
		[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: soft count deadline dialog with a $displayTimeoutSECONDS second timeout."
		[[ -z $softDisplayTimeoutSECONDS ]] && sendToLog "IBM Notifier: soft count deadline dialog with no timeout."
	...
	# Variations for the main body text of the soft deadline dialog
	if [[ "$deadlineDateSTATUS" == "SOFT" ]]; then
		[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: soft date deadline dialog with a $displayTimeoutSECONDS second timeout."
		[[ -z $softDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: soft date deadline dialog with no timeout."
		jamfHelperARRAY+=(-description "$dialogSoftDeadlineBodyDateJAMF")
	elif [[ "$deadlineDaysSTATUS" == "SOFT" ]]; then
		[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: soft days deadline dialog with a $displayTimeoutSECONDS second timeout."
		[[ -z $softDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: soft days deadline dialog with no timeout."
		jamfHelperARRAY+=(-description "$dialogSoftDeadlineBodyDaysJAMF")
	elif [[ "$deadlineCountSTATUS" == "SOFT" ]]; then
		[[ -n $softDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: soft count deadline dialog with a $displayTimeoutSECONDS second timeout."
		[[ -z $softDisplayTimeoutSECONDS ]] && sendToLog "jamfHelper: soft count deadline dialog with no timeout."
...
if [[ "$deadlineDateSTATUS" == "SOFT" ]] || [[ "$deadlineDateSTATUS" == "HARD" ]]; then
	...
	sendToLog "IBM Notifier: Days deadline user authentication dialog with a $displayTimeoutSECONDS second timeout."
	ibmNotifierARRAY+=(-subtitle "$dialogUserAuthDeadlineDaysBODY")
elif [[ "$deadlineCountSTATUS" == "SOFT" ]] || [[ "$deadlineCountSTATUS" == "HARD" ]]; then
...
while [[ $displayTimeoutSECONDS -ge 0 ]]; do
	if [[ $userAuthATTEMPT -eq 1 ]]; then
		...
		ibmNotifierARRAY=(-type popup -always_on_top -bar_title "$dialogUserAuthTITLE" -icon_path "$cachedICON" -icon_width "$ibmNotifierIconSIZE" -icon_height "$ibmNotifierIconSIZE" -main_button_label "$restartButtonDISPLAY")
		if [[ "$deadlineDateSTATUS" == "SOFT" ]] || [[ "$deadlineDateSTATUS" == "HARD" ]]; then # Hard date deadline user authentication dialog.
			ibmNotifierARRAY+=(-subtitle "$dialogUserAuthRetryDeadlineDateBODY")
		elif [[ "$deadlineDaysSTATUS" == "SOFT" ]] || [[ "$deadlineDaysSTATUS" == "HARD" ]]; then # Hard days deadline user authentication dialog.
			ibmNotifierARRAY+=(-subtitle "$dialogUserAuthRetryDeadlineDaysBODY")
		elif [[ "$deadlineCountSTATUS" == "SOFT" ]] || [[ "$deadlineCountSTATUS" == "HARD" ]]; then # Hard count deadline user authentication dialog.
...
if [[ "$userAuthREADY" == "TRUE" ]]; then
	...
	if [[ "$bootstrapTOKEN" != "TRUE" ]] && { [[ "$userAuthMDMFAILOVER" == "TRUE" ]] || [[ "$userAuthMDMFailoverBOOTSTRAP" == "TRUE" ]]; }; then
...
[[ "$forceRestartOPTION" == "TRUE" ]] && sendToLog "Warning: This computer will restart even if there is no macOS update/upgrade availble."
...
if [[ "$skipUpdatesOPTION" == "TRUE" ]]; then # Skip software updates/upgrade mode option.
	...
	if [[ "$testModeOPTION" == "TRUE" ]]; then
		...
		{ [[ "$forceRestartOPTION" != "TRUE" ]] && [[ -z $policyTRIGGERS ]]; } && sendToLog "Warning: You need to also use --force-restart or --policy-triggers to simulate notification and dialog workflows."
	fi
	...
	if [[ "$testModeOPTION" != "TRUE" ]]; then
		...
		macOSUpgradeVersionTARGET="FALSE"
		softwareUpdateMACOS="FALSE"
		if [[ "$enforceAllUpdatesOPTION" == "TRUE" ]]; then
			softwareUpdateRECOMMENDED="TRUE"
			sendToLog "Test Mode: Simulating enforce all updates workflow."
		...
		elif [[ "$forceRestartOPTION" == "TRUE" ]]; then
			sendToLog "Test Mode: Simulating force restart workflow."
	...
	if [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]]; then # A macOS upgrade is available and option to allow upgrade is enabled.
		...
		defaults delete "$superPLIST" SoftCounter 2> /dev/null
		defaults delete "$superPLIST" HardCounter 2> /dev/null
		if [[ "$enforceAllUpdatesOPTION" == "TRUE" ]]; then # Option to enforce all software updates is enabled.
			if [[ "$installNowOPTION" == "TRUE" ]]; then
				sendToLog "**** S.U.P.E.R.M.A.N. $superVERSION INSTALL NOW UPDATE RECOMMENDED SOFTWARE ****"
				installRecommendedWorkflow # This function includes internal install now and test mode logic.
			elif [[ "$onlyDownloadOPTION" == "TRUE" ]]; then
				sendToLog "Warning: The --enforce-all-updates option is ignored when used with the --only-download option."
			else # Default super workflow.
				sendToLog "**** S.U.P.E.R.M.A.N. $superVERSION UPDATE RECOMMENDED SOFTWARE ****"
				installRecommendedWorkflow # This function includes internal install now and test mode logic.
			fi
		else
			sendToLog "Status: Only recommended (non-macOS) software udpates are availble. You must use the --enforce-all-updates option for these items to install without a macOS update/upgrade."
		fi
...
# This is the main logic for determining what to do in the case of available macOS updates, macOS upgrades, $policyTRIGGERS, or the $forceRestartOPTION.
if [[ "$softwareUpdateMACOS" == "TRUE" ]] || [[ "$macOSUpgradeVersionTARGET" != "FALSE" ]] || [[ -n $policyTRIGGERS ]] || [[ "$forceRestartOPTION" == "TRUE" ]]; then
